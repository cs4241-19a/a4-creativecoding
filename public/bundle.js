...
var FizzyText = function FizzyText() {
  ...
  this.explode = function () {}; // Define render logic ...
...
ctx = canvas.getContext("2d"); // find the center of the window

center_x = canvas.width / 2;
center_y = canvas.height / 2;
radius = 150; //draw a circle
...
function animationLooper() {
  ...
  center_x = canvas.width / 2;
  center_y = canvas.height / 2;
  radius = 150; // style the background

  var gradient = ctx.createLinearGradient(0, 0, 0, canvas.height);
  gradient.addColorStop(0, "rgba(35, 7, 77, 1)");
  gradient.addColorStop(1, "rgba(204, 83, 51, 1)");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, canvas.width, canvas.height); //draw a circle
  ...
  for (var i = 0; i < bars; i++) {
    //divide a circle into equal parts
    rads = Math.PI * 2 / bars;
    bar_height = frequency_array[i] * 0.7; // set coordinates
...
function setup() {
  ...
  var pieces = 32; // Circle's radius

  var radius = 200; // Move the origin to the center of the canvas

  translate(width / 2, height / 2); // The centered circle

  stroke(0, 0, 255);
  ellipse(0, 0, radius); // For each piece draw a line

  for (i = 0; i < pieces; i++) {
    // Rotate the point of origin
    rotate(TWO_PI / pieces); // Draw the red lines

    stroke(255, 0, 0);
    line(10, radius / 2, 0, radius); //Optionally also draw to the opposite direction
